...
updateLAIandStocks.WBveg <- function(WBveg,modeling_options) {
  ...
  WBveg$Q_LSym_sat_L <- (1 / (WBveg$params$LDMC / 1000) - 1) * WBveg$DMLiveCanopy * (1 - WBveg$params$apoFrac_Leaf) / 1000 # Leaf symplastic water content in l/m2 (i.e. mm)
  ...
  WBveg$Q_LApo_sat_L <- (1 / (WBveg$params$LDMC / 1000) - 1) * WBveg$DMLiveCanopy * (WBveg$params$apoFrac_Leaf) / 1000 # Leaf symplastic water content in l/m2 (i.e. mm)
...
compute.waterStorage.WBveg <- function(WBveg, VPD)
{
  ...
  WBveg$LFMCSymp <- 100 * (Q_LSym / (WBveg$DMLiveCanopy * (1 - WBveg$params$apoFrac_Leaf) / 1000))
  
  #---Apoplasmic water content of the leaves-------------------------------------------------
  Q_LApo = (1-WBveg$PLC_Leaf/100) *  WBveg$Q_LApo_sat_L
  WBveg$Q_LApo_L <- Q_LApo
  WBveg$LFMCApo <- 100 * (Q_LApo / (WBveg$DMLiveCanopy * WBveg$params$apoFrac_Leaf / 1000)) #  LFMC of Apo (relative moisture content to dry mass), gH20/gMS
...
compute.transpiration.WBveg <- function(WBveg, WBclim, Nhours, modeling_options) {
  ...
  if (modeling_options$transpirationModel == 'Granier') {
    ...
    # calculate Emin for Stem 
    WBveg$gCrown  = compute.gCrown(gCrown0 = WBveg$params$gCrown0, windSpeed = WBclim$WS)
    WBveg$Emin_S <-  WBveg$params$fTRBToLeaf * compute.Emin(gmin = WBveg$gmin_S, gBL=  2000, gCrown = WBveg$gCrown, VPD = WBclim$VPD) # WBveg$gBL fix at 2000 mmol/m2/s for stem
    
    
    if ( WBveg$LAIpheno > 0 ) # OR WBveg$LAI ?
    {
      # FOR LEAF
      ##########
      ...
      # For stem WBveg$Emin_S <-  WBveg$params$fTRBToLeaf * compute.Emin(gmin = WBveg$gmin_S,gBL=WBveg$gBL, gCrown=WBveg$gCrown, VPD= WBclim$VPD)
      ...
      WBveg$gmin   <- 0 # at 0 to compute Tleaf on first time step considering gs =0 and not NA s
      WBveg$Emin   <- 0
      WBveg$gs_lim <- 0 # initialised to 0 to compute Tleaf on first time step considering gs = 0 and not NA 
      WBveg$Eprime <- 0
      WBveg$Ebound <- 0
